...
echo
echo "This set is embedded into the bootimus binary via go:embed — commit the"
echo "refreshed $OUT_DIR and rebuild. Select it via the"
echo "bootloader-sets API/UI. The built-in proxyDHCP advertises the manifest's"
echo "bootfiles automatically. External DHCP servers must point UEFI clients at"
echo "ipxe-shimx64.efi themselves."
